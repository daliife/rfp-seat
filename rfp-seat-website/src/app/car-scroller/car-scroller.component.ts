...
export class CarScrollerComponent implements OnInit {
  ...
  private refreshPercentage(): number {
    ...
    const scrollPositionRelative = (scrollPosition - homeHeight) / (documentHeight - viewportHeight - homeHeight);
    const percentageResult = (scrollPosition - $('#home-container').height() ) < 0 ? 0 : scrollPositionRelative;
